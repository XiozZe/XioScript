...
const Formulas = Object.freeze({
	...
	salary: (salaryNew, salaryCity, skillNow, skillCity, skillRequired) => {
		let calc = 0; 
		if(salaryNew > salaryCity)
			calc = skillNow - skillCity * Math.log( 1 + salaryNew / salaryCity ) / Math.log(2);
		else
			calc = Math.pow( salaryCity / salaryNew , 2) * skillNow - skillCity;	

		if(skillRequired > calc+skillCity)
			return salaryCity * (Math.pow(2, ( skillRequired - calc ) / skillCity ) - 1);
		else
			return salaryCity * Math.sqrt( skillRequired / ( skillCity + calc ) );			
	},
